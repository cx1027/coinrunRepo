...
//Todo:first 2 is agent, next 2 is monster
static char condition[8];
static float agentValue[2];
static float monsterArray[10];
static float coinArray[2];
static float wallArray[2];
static char wallChar[4096];
static int set_curr_x_value[1];
...
const char NON_MARKOV1_SURFACE = 'T';
const char NON_MARKOV2_SURFACE = 'N';
...
//    printf("wallChar\n");
  ...
  int set_elem_non_markov(int x, int y, int val)
  {
    return wallChar[w*y + x] = val;
  }
...
//    fill_block_top(x, y, dx, dy, WALL_MIDDLE, NON_MARKOV1_SURFACE);
//    fill_block_top(x, y, dx, dy, WALL_MIDDLE, NON_MARKOV2_SURFACE);
  }
  ...
  {
    maze->fill_elem(0, 0, maze->w, maze->h, SPACE);
    std::cout<<"maze->w:"<<maze->w<<" maze->h: "<<maze->w<<"SPACE"<<SPACE<<std::endl;
//    printf("\n initial_floor_and_walls*****************\n");
    printf("maze->initial_floor_and_walls();\n");
    printf("walls0\n");
    maze->printWalls();
    maze->printWallchar();
    printf("\nfinishwalls0\n");
  ...
  void generate_coin_to_the_right(int game_type)
  {
    ...
    //TODO:make it longer
    int curr_x = currx;
    std::cout << "currxcurrx: " << currx << std::endl;
...
//    int curr_x = 14;
    ...
    //TODO:SET SECOND COIN
//    maze->set_elem(curr_x-8, curr_y, COIN_OBJ2);
//    maze->set_elem(curr_x-7, curr_y+7, COIN_OBJ2);
...
extern "C" void getConditionWithXY(int x, int y)
  {
//    printf("!!!!!!!!condition!!!!");
//    std::shared_ptr<Maze> maze;


    condition[0] = wallChar[64*(y-1) + (x-1)];
//    maze->get_elem(int(x-1), int(y-1));
    condition[1] = wallChar[64*(y-1) + x];
    condition[2] = wallChar[64*(y-1) + (x+1)];
    condition[3] = wallChar[64*y + (x-1)];
    condition[4] = wallChar[64*y + (x+1)];
    condition[5] = wallChar[64*(y+1) + (x-1)];
    condition[6] = wallChar[64*(y+1) + x];
    condition[7] = wallChar[64*(y+1) + (x+1)];
  }
...
//    printf("monitor_csv_episode_over\n");
    if (!monitor_csv)
      return;
...
//    printf("\n initial_non_markov*****************\n");
//    maze->set_elem_non_markov(2, 0, NON_MARKOV1_SURFACE);
//    maze->set_elem_non_markov(6, 2, NON_MARKOV2_SURFACE);
//    maze->set_elem_non_markov(10, 7, NON_MARKOV1_SURFACE);
// todo: value=13,works for 14
    maze->set_elem_non_markov(13, 0, NON_MARKOV1_SURFACE);
// todo: value=3,works for 4
    maze->set_elem_non_markov(non_markov_block, 0, NON_MARKOV1_SURFACE);
    maze->set_elem_non_markov(6, 4, NON_MARKOV2_SURFACE);
    maze->set_elem_non_markov(7, 3, NON_MARKOV1_SURFACE);
    maze->set_elem_non_markov(1, 3, NON_MARKOV1_SURFACE);
    maze->set_elem_non_markov(4, 4, NON_MARKOV1_SURFACE);
    maze->set_elem_non_markov(4, 7, NON_MARKOV1_SURFACE);
    maze->set_elem_non_markov(9, 5, NON_MARKOV2_SURFACE);
    maze->set_elem_non_markov(8, 10, NON_MARKOV2_SURFACE);
...
//    std::cout<<"\n*******************************in step_coinrun"<<"\n";
//
//    std::cout<<"\n step_coinrun agent x:"<<x<<"\n step_coinrun agent y:"<<y<<"\nint agent x:"<<ceil(x)<<"\nint agent y:"<<ceil(y)<<"\n";

//    condition[0] = maze->get_elem(ceil(x-1), ceil(y-1));
//    condition[1] = maze->get_elem(ceil(x), ceil(y-1));
//    condition[2] = maze->get_elem(ceil(x+1), ceil(y-1));
//    condition[3] = maze->get_elem(ceil(x-1), ceil(y));
//    condition[4] = maze->get_elem(ceil(x+1), ceil(y));
//    condition[5] = maze->get_elem(ceil(x-1), ceil(y+1));
//    condition[6] = maze->get_elem(ceil(x), ceil(y+1));
//    condition[7] = maze->get_elem(ceil(x+1), ceil(y+1));
//    int xx=ceil(x);
//    int yy=ceil(y);
//    agentValue[0]=xx;
//    agentValue[1]=yy;
//    condition[0] = wallChar[64*(yy-1) + (xx-1)];
//    condition[1] = wallChar[64*(yy-1) + xx];
//    condition[2] = wallChar[64*(yy-1) + (xx+1)];
//    condition[3] = wallChar[64*yy + (xx-1)];
//    condition[4] = wallChar[64*yy + (xx+1)];
//    condition[5] = wallChar[64*(yy+1) + (xx-1)];
//    condition[6] = wallChar[64*(yy+1) + xx];
//    condition[7] = wallChar[64*(yy+1) + (xx+1)];
//
//    printf("condition1");
//    std::cout<<condition[0]<<condition[1]<<condition[2]<<condition[3]<<condition[4]<<condition[5]<<condition[6]<<condition[7]<<std::endl;

//    getConditionWithXY(x,y);
//    printf("condition2");
//    std::cout<<condition[0]<<condition[1]<<condition[2]<<condition[3]<<condition[4]<<condition[5]<<condition[6]<<condition[7]<<std::endl;
...
//      printf("\nbefore monster move in for loop:\n");
      int monster_i = 0;
...
//        std::cout<<"monster positon: x: "<<m->x<<" y:"<<m->y<<"\n";
        //todo: how to identify i in for loop

        monsterArray[monster_i] = m->x;
        monsterArray[monster_i+1] = m->y;
        monster_i+=2;
      }
      monster_i =0;
//      std::cout<<"monsterArray[0]: "<<monsterArray[0]<<" monsterArray[1]: "<<monsterArray[1]<<"monsterArray[2]: "<<monsterArray[2]<<" monsterArray[3]: "<<monsterArray[3]<<"\n";
//      printf("\nafter monster move in for loop:\n");
      //todo:monsterArray[i]清零
...
float* get_agentValue()  { return agentValue; }
...
int vec_create(int game_type, int nenvs, int lump_n, bool want_hires, float default_zoom, int setx, int setnonmarkov)
{
//  printf("vec_create!: ");
  ...
  currx = setx;
  non_markov_block = setnonmarkov;
  std::cout << "non_markov_block: "<< non_markov_block << std::endl;
...
//      std::cout << "actions[e]"<<actions[e] << std::endl;
      state->agent.action_dx = DISCRETE_ACTIONS[2 * actions[e] + 0];
//      std::cout << "state->agent.action_dx"<<state->agent.action_dx << std::endl;

      state->agent.action_dy = DISCRETE_ACTIONS[2 * actions[e] + 1];
//      std::cout << "state->agent.action_dy"<<state->agent.action_dy << std::endl;
      {
...
//      printf("******action:%d, agent:dx:dy:x:y = %d:%d:%f:%f\n",*actions, state->agent.action_dx, state->agent.action_dy,state->agent.x,state->agent.y);
    }
  }
  wait_for_actions.wakeAll();
//  printf("***********finish action:\n");
...
//    std::cout<<"rew[e]: "<<a.reward<<" done[e]: "<<done[e]<<" donedonedone\n";
    a.reward = 0;
    a.game_over = false;
    agent[0]=a.x;
    agent[1]=a.y;
//    std::cout<<"vec_wait ax:"<<a.x<<" vec_wait ay"<<a.y<<"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxa\n";
    int xx=ceil(a.x);
    int yy=ceil(a.y);
    agentValue[0]=xx;
    agentValue[1]=yy;
...
extern "C" float get_agent(int handle)
{
    float agent[2];
//    float *agent=new float[2];
//    printf("\nget_agent\n");
//    std::cout<<"state_e->agent: ["<<state_e->agent<<"####################\n";
    std::shared_ptr<VectorOfStates> vstate = vstate_find(handle);
    for (int e = 0; e < vstate->nenvs; e++) {
        std::shared_ptr<State> state_e = vstate->states[e];
        QMutexLocker lock2(&state_e->state_mutex);
        ...
        agent[1] = a.y;
        //
//        std::cout<<"a.x: ["<<agent[0]<<"] a.y: ["<<agent[1]<<"]####################\n";
//        std::cout<<"*a.x: ["<<*agent[0]<<"]####################\n";
...
extern "C" void get_agent_array(int handle, float *position)
{
...
//    std::cout<<"state_e->agent: ["<<state_e->agent<<"####################\n";
    ...
    for (int e = 0; e < vstate->nenvs; e++) {
        ...
        position[1] = 2.1;//a.y;
        //
//        std::cout<<"get_agent_array:a.x: ["<<position[0]<<"] a.y: ["<<position[1]<<"]####################\n";
//        std::cout<<"*a.x: ["<<*agent[0]<<"]####################\n";

    }
//    std::cout<<*position<<" outpu \n";
    //agent是数组首元素的首地址
    //return position;
}

extern "C" int* functionA(){
...
extern "C" int get_number(int handle)
{
    return 11;
}

//extern "C" int* get_array(int handle)
//{
//  int agent[2] = {1, 1};
//  printf("\nget_agent:" + agent[1]);
//  return agent;
...
public:
  ...
  void timeout()
  {
    ...
    float agent[2];
    float conditon[2];
    vec_wait(viz->control_handle, bufrgb, 0, bufrew, bufdone,conditon);
    fprintf(stderr, "%+0.2f %+0.2f %+0.2f\n", bufrew[0], bufrew[1], bufrew[2]);
  }
...
extern "C" void test_main_loop()
{
  ...
  int handle = vec_create(DEFAULT_GAME_TYPE, 1, 0, false, 5.0, currx, non_markov_block);
